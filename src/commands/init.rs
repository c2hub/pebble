...
use recipe_reader::{Recipe, Target, TargetOptions, TargetType};
...
pub fn init_pebble(path_str: &str, kind: PebbleType) {
	...
	if proj_path.exists() {
		...
		for f in match read_dir(Path::new(".")) {
			Ok(r) => r,
			_ => fail("failed to open current directory", 25),
		} {
			match f {
				Ok(f) => {
					if match f.metadata() {
					...
					} && match Path::new(&f.path()).extension() {
						Some(ex) => ex == "c2",
						None => false,
					} {
						let path = f.path();
						...
						if copy(
							Path::new(filename),
							Path::new(&("src/".to_string() + filename)),
						).is_err()
						{
