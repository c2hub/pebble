...
pub fn scan() {
	...
	for f in match read_dir(Path::new("src")) {
		Ok(r) => r,
		_ => fail("failed to open source directory", 84),
	} {
		let file = match f {
			Ok(fl) => fl,
		...
		if !recipe.targets[0]
			.files
			.contains(&file.path().to_string_lossy().deref().to_string())
		{
			for t in &mut recipe.targets {
				t.files
					.insert(0, file.path().to_string_lossy().deref().to_string());
			}
			println!(
				"  {} {}",
	...
	for t in &mut recipe.targets {
		for file in t.files.clone() {
			if !Path::new(&file).exists() {
				...
				println!("  {} {}", Red.bold().paint("removing"), file,);
			}
		}
	}
