...
pub fn upload() {
	...
	let res = Packet::upload(
		...
		(len - (len % PACKET_BYTES)) / PACKET_BYTES + (if len % PACKET_BYTES != 0 { 1 } else { 0 }),
		&cfg.pebble.name,
		&cfg.pebble.version,
	).send();
	...
	match res {
		...
		Packet::Upload { parts, .. } => {
			...
			// just for clarity
			let port = parts;
			let parts = (len - (len % PACKET_BYTES)) / PACKET_BYTES + (if len % PACKET_BYTES != 0 {
				1
			} else {
				0
			});
			...
			println!("  {} established", Yellow.bold().paint("connection"));

			println!("parts: {}", parts);
			match parts {
				0 => fail("the data vector is empty", 865),
				1 => match Packet::transfer(1, bytes).send_to(&socket) {
					Packet::Transfer { .. } => {
						println!("  {} successful", Yellow.bold().paint("transfer"));
					}
					Packet::Error { msg } => fail1("packet -> {}", msg, 110),
					_ => unreachable!(),
				},
				_ => {
					...
					loop {
						match Packet::transfer(current_part, current_bytes.clone()).send_to(&socket)
						{
