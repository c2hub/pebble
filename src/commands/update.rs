...
pub fn update() {
	...
	if Recipe::find() != None {
		_recipe.read();
		let _ = set_current_dir(Path::new(&_recipe.path.parent().unwrap()));
	} else {
	...
	println!(
		"  {} http://magnusi.tech/static/pebbles/data for pebbles",
...
pub fn fetch_deps(deps: Vec<Dependency>) {
	for dep in deps {
		...
		if find_ver(&format!("lib{}", &dep.name), &dep.version) {
			let last_ver = match Packet::find(&format!("lib{}", &dep.name), &dep.version).send() {
				...
				.send()
			{
				Ok(res) => res,
				Err(_) => fail(
					"failed to acquire pebble, are you connected to the internet?",
					4521,
				),
			};
...
pub fn unpack_deps() {
	for f in match read_dir(Path::new("libs")) {
		Ok(r) => r,
		_ => fail("failed to open source directory", 451),
	} {
		let file = match f {
			Ok(fl) => fl,
		...
		for i in 0..archive.len() {
			if let Ok(mut f) = archive.by_index(i) {
				...
				fail("failed to read zip entry", 4613);
			}
		}
